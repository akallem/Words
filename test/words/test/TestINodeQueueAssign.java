...
public class TestINodeQueueAssign extends TestINode {
	...
	public void queueAssignShouldWorkForOneProperty() throws WordsRuntimeException {
		...
		assertEquals("Property does not currently exist", environment.getVariable("Fred").objProperty.getProperty(prop1Name).type, Variable.VariableType.NOTHING);
		
		INodeQueueAssign testNode = new INodeQueueAssign(refList, onePropList, null);
		loop.enqueueAST(testNode);
		assertEquals("Property does not exist after enqueueing", environment.getVariable("Fred").objProperty.getProperty(prop1Name).type, Variable.VariableType.NOTHING);		
	...
	public void queueAssignShouldSimultaneouslyAssignMultipleProperties() throws WordsRuntimeException {
		environment.createObject("Fred", "thing", new Position(0,0));
		loop.fastForwardEnvironment(1); //object is created with a 1 frame wait, so use it up.
		assertEquals("Property 1 does not currently exist", environment.getVariable("Fred").objProperty.getProperty(prop1Name).type, Variable.VariableType.NOTHING);
		assertEquals("Property 2 does not currently exist", environment.getVariable("Fred").objProperty.getProperty(prop2Name).type, Variable.VariableType.NOTHING);
		assertEquals("Property 3 does not currently exist", environment.getVariable("Fred").objProperty.getProperty(prop3Name).type, Variable.VariableType.NOTHING);
		
		INodeQueueAssign testNode = new INodeQueueAssign(refList, threePropList, null);
		loop.enqueueAST(testNode);
		assertEquals("Property 1 does not exist after enqueueing", environment.getVariable("Fred").objProperty.getProperty(prop1Name).type, Variable.VariableType.NOTHING);
		assertEquals("Property 2 does not exist after enqueueing", environment.getVariable("Fred").objProperty.getProperty(prop2Name).type, Variable.VariableType.NOTHING);
		assertEquals("Property 3 does not exist after enqueueing", environment.getVariable("Fred").objProperty.getProperty(prop3Name).type, Variable.VariableType.NOTHING);
	...
	public void testQueueAssignWithNow() throws WordsRuntimeException {
		...
		loop.fastForwardEnvironment(1);
		assertEquals("Property 1 does not exist after one frame", environment.getVariable("Fred").objProperty.getProperty(prop1Name).type, Variable.VariableType.NOTHING);
